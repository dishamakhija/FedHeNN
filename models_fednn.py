...
## Contains NNs with different architectures used to instantiate clients
...
class RNNSent(nn.Module):
    ...
    def get_hidden_representation(self, input, hidden, l):
        input = torch.transpose(input, 0,1)
        #print(input.shape)
        emb = torch.zeros((25,l,300))
        for i in range(25):
            for j in range(l):
                emb[i,j,:] = self.encoder[input[i,j],:]
        emb = emb.to(self.device)
        emb = emb.view(300,l,25)
        self.rnn.flatten_parameters()
        output, hidden = self.rnn(emb, hidden)
        return hidden
...
class CNNCifar1(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 64 * 14 * 14)
        x = F.relu(self.fc1(x))
        feat_vec = (F.relu(self.fc2(x)))
        x1 = self.drop(feat_vec)
        label = F.log_softmax(x1, dim=1)
...
class CNNCifar100(nn.Module):
    ...
    def forward(self, x):
        ...
        feat_vec = x
        x1 = self.fc3(feat_vec)
        label = F.log_softmax(x1, dim=1)
        return feat_vec, label


class CNNCifar100_1(nn.Module):
    def __init__(self, args):
        super(CNNCifar100_1, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, 5)
        self.pool = nn.MaxPool2d(2, 2)
        self.drop = nn.Dropout(0.6)
        self.conv2 = nn.Conv2d(64, 128, 5)
...
class CNNCifar100_2(nn.Module):
    def __init__(self, args):
        super(CNNCifar100_2, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, 5)
        self.pool = nn.MaxPool2d(2, 2)
        self.drop = nn.Dropout(0.6)
        self.conv2 = nn.Conv2d(64, 128, 5)
...
class MLP5(nn.Module):
    def __init__(self, dim_in, dim_hidden, dim_out):
        ...
        self.layer_hidden2 = nn.Linear(256, 256)
        self.layer_hidden3 = nn.Linear(256, 128)
        self.layer_hidden4 = nn.Linear(256, 128)
        self.layer_hidden4 = nn.Linear(128, 64)
        self.layer_hidden5 = nn.Linear(64, 32)
        self.layer_out = nn.Linear(32, dim_out)
        self.softmax = nn.Softmax(dim=1)
        self.weight_keys = [['layer_input.weight', 'layer_input.bias'],
                            ['layer_hidden1.weight', 'layer_hidden1.bias'],
                            ['layer_hidden2.weight', 'layer_hidden2.bias'],
                            ['layer_hidden3.weight', 'layer_hidden3.bias'],
                            ['layer_hidden4.weight', 'layer_hidden4.bias'],
                            ['layer_hidden5.weight', 'layer_hidden5.bias'],
                            ['layer_out.weight', 'layer_out.bias']
                            ]
...
class CNNMnist(nn.Module):
    def __init__(self, args):
        ...
        self.fc3 = nn.Linear(64, args.num_classes)
        self.weight_keys = [['fc1.weight', 'fc1.bias'], ['fc2.weight', 'fc2.bias'],['fc3.weight', 'fc3.bias'],['conv2.weight', 'conv2.bias'],['conv1.weight', 'conv1.bias']]
