...
class LocalUpdate(object):
    def __init__(self, args, l=None, dataset=None, idxs=None, indd=None, sample_images = None, eta = None):
        self.args = args
    ...
    def train(self, net, net_glob_last, w_glob_keys, last=False, dataset_test=None, ind=-1, idx=-1, lr=0.01):
        bias_p=[]
        weight_p=[]
        ...
        for iter in range(local_eps):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                if 'sent140' in self.args.dataset:
                    ...
                    preds, reps_global = net_glob_last(self.sample_images.to(self.args.device))
                    preds, reps_local = net(self.sample_images.to(self.args.device))
...
parser.add_argument('--num_users', type=int, default=400, help="number of users: n")
...
run_comment = args.dataset + '_' + args.model + '_' + str(args.num_users)+ 'users_' + str(args.shard_per_user) + '_classes_' + '_l' + str(l)
outfile = '/home/disha/Documents/fednn/fednn_scripts/results/homo_lin_cka_'+ run_comment + date_time + '.txt'
...
for eta0 in [0.01]:
    ...
    for iter in range(args.epochs+1):
            ...
            for ind, idx in enumerate(idxs_users):
                ...
                if 'femnist' in args.dataset or 'sent140' in args.dataset:
                    if args.epochs == iter:
                        local = LocalUpdate(args=args,l=l, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_ft]], idxs=dict_users_train, indd=indd, sample_images = sample_images, eta=eta)
                    else:
                        local = LocalUpdate(args=args,l=l, dataset=dataset_train[list(dataset_train.keys())[idx][:args.m_tr]], idxs=dict_users_train, indd=indd, sample_images = sample_images,  eta=eta)
                else:
                    if args.epochs == iter:
                        local = LocalUpdate(args=args,l=l, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_ft], sample_images = sample_images,  eta=eta)
                    else:
                        local = LocalUpdate(args=args,l=l, dataset=dataset_train, idxs=dict_users_train[idx][:args.m_tr], sample_images = sample_images,  eta=eta)
                ...
                if 'femnist' in args.dataset or 'sent140' in args.dataset:
                    w_local, loss, indd = local.train(net=net_local.to(args.device),net_glob_last=net_glob, ind=idx, idx=clients[idx], w_glob_keys=w_glob_keys, lr=args.lr,last=last)
                else:            
                    w_local, loss, indd = local.train(net=net_local.to(args.device),net_glob_last=net_glob, idx=idx, w_glob_keys=w_glob_keys, lr=args.lr, last=last)
            ...
            if iter % args.test_freq==args.test_freq-1 or iter>=args.epochs-10:
                ...
                if iter != args.epochs:
                    ...
                    writer.writerow('Round {:3d}, Train loss: {:.3f}, Test loss: {:.3f}, Test accuracy: {:.2f}'.format(
                            iter, loss_avg, loss_test, acc_test))
                    ...
                    writer.writerow('Final Round, Train loss: {:.3f}, Test loss: {:.3f}, Test accuracy: {:.2f}'.format(
                            loss_avg, loss_test, acc_test))
                ...
                if args.alg == 'fedavg' or args.alg == 'prox':
                    ...
                    if iter != args.epochs:
                        print('Round {:3d}, Global train loss: {:.3f}, Global test loss: {:.3f}, Global test accuracy: {:.2f}'.format(
                            iter, loss_avg, loss_test, acc_test))
                        writer.writerow('Round {:3d}, Global train loss: {:.3f}, Global test loss: {:.3f}, Global test accuracy: {:.2f}'.format(
                            iter, loss_avg, loss_test, acc_test))

                    else:
                        print('Final Round, Global train loss: {:.3f}, Global test loss: {:.3f}, Global test accuracy: {:.2f}'.format(
                            loss_avg, loss_test, acc_test))
                        writer.writerow('Final Round, Global train loss: {:.3f}, Global test loss: {:.3f}, Global test accuracy: {:.2f}'.format(
                            loss_avg, loss_test, acc_test))
                ...
                if iter % args.save_every==args.save_every-1:
                    model_save_path = './models/homo_lin_accs_'+ args.alg + '_' + args.dataset + '_' + str(args.num_users) +'_'+ str(args.shard_per_user) +'_iter' + str(iter)+ '.pt'
                    torch.save(net_glob.state_dict(), model_save_path)


    writer.writerow('Average accuracy final 10 rounds: {}'.format(accs10))
    if args.alg == 'fedavg' or args.alg == 'prox':
      writer.writerow('Average global accuracy final 10 rounds: {}'.format(accs10_glob))
    end = time.time()
